...
- [ ] **Injury status** — requested, not yet built, awaiting user confirmation on approach. ESPN's `kona_player_info` response (already fetched by `ingest_espn.py`) includes `player.injuryStatus` per player — no new API calls needed, just capture a field we're currently ignoring. Recommended design (mirrors the `my_rank`/`overall_rank` split): a separate pipeline-refreshed `injury_status` column on `players`, NOT mixed into the user's manual `tags` array, displayed as its own badge — since ESPN-sourced data conflated with user-curated data is exactly the bug that split caused. Needs the user's go-ahead before implementing.
