...
- [x] **`my_rank` vs `overall_rank` split** — (2026-08-14) User feedback after first hands-on look: `overall_rank` (ESPN's imported rank, refreshed every ingestion run) and the user's personal manual rank were conflated in one field, meaning drag-reorder was silently overwritable by a future pipeline re-run. Added a separate `my_rank` column (`supabase/schema.sql`, migration block — **user needs to run this in the SQL editor on the live project**, I don't have DDL access via the API keys I was given). Sort order is now `my_rank` with `overall_rank` as fallback (both in the SSR query in `page.tsx` and client-side `byRank` in `PlayerBoard.tsx`); drag-and-drop now only ever writes `my_rank`. UI shows the user's rank as the big draggable number and ESPN's rank + ADP as a small reference label. **Not yet verified against live data** — blocked on the user running the migration; code is done and builds clean.
- [x] **Performance: pagination + row memoization** — (2026-08-14) User reported the 1000-player ALL view was "super laggy" on hide-drafted toggle, draft button, and drag-and-drop, while smaller tabs (DST, ~30 players) felt fine — confirms rendered/draggable row count was the bottleneck, not data-layer cost. `PlayerBoard.tsx` now paginates at 100/page (Prev/Next, resets to page 1 on filter change) and extracts row rendering into a `React.memo`'d `PlayerRow` so an unrelated state change (e.g. toggling one player's `is_drafted`) doesn't re-render every other row — relies on `visiblePlayers`/`pagePlayers` preserving object identity for untouched players, which `.filter()`/`.slice()` already do. Drag reorder accounts for the page offset when mapping local Draggable index back to the global array. **Not yet re-verified for actual perceived smoothness** — needs a hands-on check once the `my_rank` migration is applied (same deploy).
- [ ] **DST "rank changes" report** — (2026-08-14) User saw the rank number appear to change when drafting a DST with hide-drafted on. Investigated: confirmed ESPN's `overall_rank` values for DST have no ties/duplicates, so that's not the cause. Best current theory (unconfirmed): with hide-drafted on, drafting a player removes it from the visible list, so *other* players' rows shift position — under the old laggy re-render this could plausibly have looked like numbers "changing" even though each player's own stored rank didn't move. The `my_rank`/pagination/memoization changes above may resolve this as a side effect (smoother renders, clearer labeling of "your rank" vs ESPN's). Ask the user to re-check after this deploy; get a screen recording if it still reproduces.
