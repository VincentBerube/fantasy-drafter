...
- [x] **Tag/notes editing** — (2026-08-14) Tags and notes were display-only until now. `PlayerBoard.tsx` adds `TagPicker` (a `<details>`-based dropdown listing the 4 preset tags — `Star`/`Target`/`Sleeper`/`Avoid` — not yet on the player) and `TagPill` now has a remove ("×") button; both write through `toggleTag` (optimistic update + Supabase `.update()`). `NotesEditor` is an always-visible inline text input (placeholder "Add note…") that saves on blur via `updateNotes`, same write-through pattern; resyncs its local value from the `notes` prop if it changes externally (e.g. another device) using the render-time state-adjustment pattern, not `useEffect`, per the project's `react-hooks/set-state-in-effect` lint rule. Verified end-to-end: built clean, confirmed the picker/editor render in SSR output, and round-tripped a real tag+note write through the REST API against the live table. Tag vocabulary is currently fixed to the 4 presets — no free-text custom tags yet.
